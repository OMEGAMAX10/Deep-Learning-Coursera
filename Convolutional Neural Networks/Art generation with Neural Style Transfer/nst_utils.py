# Part of this code is due to the MatConvNet team and is used to load the parameters of the pretrained VGG19 model in
# the notebook
import scipy.io
import numpy as np
...
def save_image(path, image):
    ...
    image = Image.fromarray(np.clip(image[0], 0, 255).astype('uint8'))
    image.save(path)
...
def show_image(generated_image):
    ...
    # Clip and Save the image
    gen_img = Image.fromarray(np.clip(generated_image[0], 0, 255).astype('uint8'))
    gen_img.show()
